...
def inject_global_styles():
    ...
    .measurement-summary-card{
        ...
        padding:12px 13px;
        margin-bottom:9px;
        box-shadow:0 7px 18px rgba(31,58,120,.05);
    }

    .measurement-summary-card.length{border-left:5px solid #f97316;}
    .measurement-summary-card.average{border-left:5px solid #22c55e;}
    .measurement-summary-card.maximum{border-left:5px solid #3b82f6;}
    .measurement-summary-card.neutral{border-left:5px solid #94a3b8;}
...
def weighted_skeleton_length(skeleton):
    ...
    horizontal = np.sum(sk[:, :-1] & sk[:, 1:])
    vertical = np.sum(sk[:-1, :] & sk[1:, :])
    diagonal_1 = np.sum(sk[:-1, :-1] & sk[1:, 1:])
    diagonal_2 = np.sum(sk[:-1, 1:] & sk[1:, :-1])

    return float(
        horizontal
        + vertical
        + np.sqrt(2.0) * (diagonal_1 + diagonal_2)
    )
...
def skeleton_endpoints(skeleton):
    ...
    ys, xs = np.where((sk == 1) & (neighbour_count == 1))
    return [(int(x), int(y)) for x, y in zip(xs, ys)]


def farthest_endpoint_pair(endpoints):
    if len(endpoints) < 2:
        return None
...
def measure_crack_geometry_from_mask(mask):
    ...
    radius_values = dist_map[skeleton > 0]

    avg_width_px = (
        float(np.mean(radius_values) * 2.0)
    ...
    _, _, _, max_point = cv2.minMaxLoc(dist_map)
    ...
    return {
        ...
        'orientation_deg': crack_orientation_from_skeleton(skeleton),
...
def measure_each_crack(
    ...
    for index, prediction in enumerate(predictions, start=1):
        ...
        geometry = measure_crack_geometry_from_mask(instance_mask)

        if use_scale:
            length_value = geometry['length_px'] * mm_per_pixel
            avg_width_value = geometry['avg_width_px'] * mm_per_pixel
            max_width_value = geometry['max_width_px'] * mm_per_pixel
            unit_text = 'mm'
        else:
            length_value = geometry['length_px']
            avg_width_value = geometry['avg_width_px']
...
def create_measurement_visualization(
    ...
    img_w, img_h = analyzed_pil_image.size

    records = measure_each_crack(
        predictions,
        img_w,
        img_h,
        use_scale=False,
...
def run_main_app():
    ...
    for idx, uploaded_file in enumerate(uploaded_files, start=1):
        ...
        avg_width_value=(
            ...
            / len(crack_records)
            if crack_records else 0.0
        )
        max_width_value=max(
            (item['max_width_value'] for item in crack_records),
            default=0.0,
        )
            ...
            for crack_tab, crack_item in zip(crack_tabs, crack_records):
                with crack_tab:
                    ...
                    st.markdown(
                        ...
                        <span class="chip">Unique ID: {image_id}-{crack_item["crack_id"]}</span>
                        ''',
                        unsafe_allow_html=True,
                    )

                    segmentation_col, measurement_col, summary_col = st.columns(
                        [1.1, 1.1, 0.9],
                        gap="medium",
                    )
                    ...
                    with segmentation_col:
                        st.markdown(
                            "<div class='measurement-grid-title'>Segmentation Result</div>",
                            unsafe_allow_html=True,
                        )
                        st.markdown(
                            "<div class='measurement-image-frame'>",
                            unsafe_allow_html=True,
                        )
                        st.image(analyzed_img, use_container_width=True)
                        st.markdown("</div>", unsafe_allow_html=True)
                        st.caption(
                            "Mask R-CNN segmentation used as the input "
                            "for geometric measurement."
                        )

                    with measurement_col:
                        st.markdown(
                            "<div class='measurement-grid-title'>Measurement Result</div>",
                            unsafe_allow_html=True,
                        )
                        st.markdown(
                            "<div class='measurement-image-frame'>",
                    ...
                    with summary_col:
                        st.markdown(
                            "<div class='measurement-grid-title'>Measurement Summary</div>",
                            unsafe_allow_html=True,
                        )

                        if use_scale:
                            length_secondary=f'{crack_item["length_value"]:.2f} mm'
                            avg_width_secondary=f'{crack_item["avg_width_value"]:.2f} mm'
                            max_width_secondary=f'{crack_item["max_width_value"]:.2f} mm'
                        else:
                            length_secondary='Calibration not enabled'
                            avg_width_secondary='Calibration not enabled'
                            max_width_secondary='Calibration not enabled'

                        st.markdown(
                            f'''
                            <div class="measurement-summary-card length">
                                <div class="measurement-summary-label">Centerline Length</div>
                                <div class="measurement-summary-value">{crack_item["length_px"]:.2f} px</div>
                                <div class="measurement-summary-unit">{length_secondary}</div>
                            </div>

                            <div class="measurement-summary-card average">
                                <div class="measurement-summary-label">Average Width</div>
                                <div class="measurement-summary-value">{crack_item["avg_width_px"]:.2f} px</div>
                                <div class="measurement-summary-unit">{avg_width_secondary}</div>
                            </div>

                            <div class="measurement-summary-card maximum">
                                <div class="measurement-summary-label">Maximum Width</div>
                                <div class="measurement-summary-value">{crack_item["max_width_px"]:.2f} px</div>
                                <div class="measurement-summary-unit">{max_width_secondary}</div>
                            </div>

                            <div class="measurement-summary-card neutral">
                                <div class="measurement-summary-label">Orientation</div>
                                <div class="measurement-summary-value">{crack_item["geometry"]["orientation_deg"]:.2f}°</div>
                                <div class="measurement-summary-unit">0° horizontal · 90° vertical</div>
                            </div>

                            <div class="measurement-summary-card neutral">
                                <div class="measurement-summary-label">Tortuosity</div>
                                <div class="measurement-summary-value">{crack_item["geometry"]["tortuosity"]:.3f}</div>
                                <div class="measurement-summary-unit">Centerline length / endpoint distance</div>
                            </div>
                            ''',
                            unsafe_allow_html=True,
                        )
                    ...
                    st.markdown(
                        f'''
                        <div class="status-box status-success">
                            Measurement completed for <b>{image_id}-{crack_item["crack_id"]}</b>.
                            The centerline length follows the detected crack path.
                        </div>
                        ''',
                        unsafe_allow_html=True,
                    )
